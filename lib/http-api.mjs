...
function inferExtensionFromUrl(url) {
  ...
  for (const extension of [".png", ".webp", ".gif", ".avif"]) {
...
export function parseApiRequest(requestUrl) {
  const rawQuery = requestUrl.search;
  let remoteUrl = null;

  if (rawQuery) {
    const m = rawQuery.match(/[?&]url=(.*?)(?=&(?:topStrip|radius|gifEncoder|download)=|$)/i);
    if (m) {
      remoteUrl = decodeURIComponent(m[1].replace(/\+/g, " "));
    }
  }

  if (!remoteUrl) {
    remoteUrl = requestUrl.searchParams.get("url");
    if (!remoteUrl) {
...
export async function fetchRemoteImage(sourceUrl) {
  ...
  try {
    ...
    if (!extension) {
      throw textError(415, "Only PNG, WEBP, AVIF, and GIF source images are supported.");
    }
