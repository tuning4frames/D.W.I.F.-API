...
const AVIF_SOURCE_EXTENSIONS = new Set([".avif", ".heif", ".heic"]);
...
function applyOutputFormat(pipeline, outputPath, metadata) {
  ...
  throw new Error("Unsupported output format. Use .png, .webp, .avif, or .gif.");
}
...
function runProcessCapture(executable, args) {
  return new Promise((resolve, reject) => {
    const child = spawn(executable, args, {
      windowsHide: true,
      stdio: ["ignore", "pipe", "pipe"]
...
function isAvifLikeInput(inputPath) {
  return AVIF_SOURCE_EXTENSIONS.has(path.extname(inputPath).toLowerCase());
}
...
async function probeAvifInput(inputPath) {
  const { stdout } = await runProcessCapture("ffprobe", [
    "-v",
    "error",
    "-show_entries",
    "stream=index,codec_type,nb_frames,duration,width,height,avg_frame_rate,r_frame_rate",
    "-of",
    "json",
    inputPath
  ]);
  ...
  const animatedStream = videoStreams.find((stream) => Number(stream.nb_frames || 0) > 1) ?? null;
...
async function normalizeAvifInput(inputPath) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dwif-avif-"));

  try {
    const normalizedPath = path.join(tempDir, "normalized.png");
    await runProcess("magick", [`${inputPath}[0]`, normalizedPath]);
    return { inputPath: normalizedPath, cleanupPath: tempDir, warning: null };
  } catch (error) {
    await fs.rm(tempDir, { recursive: true, force: true });
    throw new Error(`Could not decode AVIF input. ${error.message}`);
  }
}

async function prepareInputForProcessing(inputPath, avifProbe = null) {
  if (!isAvifLikeInput(inputPath)) {
    return {
      kind: "direct",
      inputPath,
      warning: null,
  ...
  if (avifProbe?.animatedStream) {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dwif-avif-seq-"));
    const framePattern = path.join(tempDir, "frame-%06d.png");
    const streamSpecifier = `${avifProbe.animatedStream.index}`;
    ...
    if (framePaths.length === 0) {
      await fs.rm(tempDir, { recursive: true, force: true });
      throw new Error("Could not decode animated AVIF frames.");
    }

    const fps =
      parseFrameRate(avifProbe.animatedStream.avg_frame_rate) ??
      parseFrameRate(avifProbe.animatedStream.r_frame_rate) ??
      30;
  ...
  const normalized = await normalizeAvifInput(inputPath);
  return {
    kind: "direct",
    inputPath: normalized.inputPath,
    warning: normalized.warning,
    cleanup: async () => {
      await fs.rm(normalized.cleanupPath, { recursive: true, force: true });
    }
  };
}

async function writeStillOutput(sourcePath, outputPath, metadata, topStrip, clampedRadius) {
  let pipeline = sharp(sourcePath, SHARP_STILL_SOURCE_OPTIONS)
    .ensureAlpha()
    .extend({
      top: topStrip,
      bottom: 0,
...
async function writeAnimatedAvifFromFrameFiles(framePattern, outputPath, fps) {
  const outputDir = path.dirname(outputPath);
  const outputExtension = path.extname(outputPath);
  const outputBaseName = path.basename(outputPath, outputExtension);
  const tempOutputPath = path.join(
    outputDir,
    `.${outputBaseName}.tmp-${process.pid}-${Date.now()}${outputExtension}`
  );
  ...
  try {
    await runProcess("ffmpeg", [
      ...
      framePattern,
      "-filter_complex",
      "[0:v]format=yuv444p[color];[1:v]format=rgba,alphaextract,format=gray[alpha]",
      "-map",
      "[color]",
      "-map",
      "[alpha]",
...
async function processAnimatedFrameSequence({
  framePaths,
  outputPath,
  manualTopStrip,
  manualRadius,
  ...
  const firstFrameMeta = await sharp(framePaths[0], SHARP_STILL_SOURCE_OPTIONS).metadata();
  if (!firstFrameMeta.width || !firstFrameMeta.height) {
  ...
  const topStrip =
    manualTopStrip ??
    getAutoValue(AUTO_TOP_STRIP_BASE, AUTO_TOP_STRIP_EXPONENT, firstFrameMeta.width, frameHeight);
  const radius =
    manualRadius ??
    getAutoValue(AUTO_RADIUS_BASE, AUTO_RADIUS_EXPONENT, firstFrameMeta.width, frameHeight);
  const imageHeight = Math.max(frameHeight - topStrip, 0);
  const clampedRadius = Math.min(radius, firstFrameMeta.width, imageHeight);
  ...
  const extension = path.extname(outputPath).toLowerCase();
  if (extension !== ".avif") {
    throw new Error("Animated AVIF input currently requires animated AVIF output.");
  }

  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dwif-avif-out-"));
  const processedPattern = path.join(tempDir, "frame-%06d.png");

  try {
    reportProgress?.(0, frameCount, "preparing");

    for (let index = 0; index < framePaths.length; index += 1) {
      const frameMeta = index === 0
        ? firstFrameMeta
        : await sharp(framePaths[index], SHARP_STILL_SOURCE_OPTIONS).metadata();
      const frameOutputPath = path.join(tempDir, `frame-${String(index + 1).padStart(6, "0")}.png`);
      await writeStillOutput(framePaths[index], frameOutputPath, frameMeta, topStrip, clampedRadius);
      reportProgress?.(index + 1, frameCount, "preparing");
    }

    await writeAnimatedAvifFromFrameFiles(processedPattern, outputPath, fps);
    reportProgress?.(frameCount, frameCount, "finishing");

    return {
      outputPath,
      width: firstFrameMeta.width,
      height: frameHeight,
      topStrip,
      ...
      animated: true,
      warning:
        firstFrameMeta.width !== REFERENCE_SIZE || firstFrameMeta.height !== REFERENCE_SIZE
          ? `Widget may look odd if the original image size is not ${REFERENCE_SIZE}x${REFERENCE_SIZE}. Detected ${firstFrameMeta.width}x${frameHeight}.`
          : null
    };
  } finally {
    await fs.rm(tempDir, { recursive: true, force: true });
...
async function writeAnimatedGifWithGifski(
  ...
  try {
    ...
    // Passing one argument per frame can exceed the platform command-line
    // limit (especially for long GIFs), causing spawn() to fail with
    // ENAMETOOLONG. Fall back to the in-process encoder in that case.
    const argumentBytes = framePaths.reduce((total, framePath) => total + Buffer.byteLength(framePath) + 1, 0);
    const maxArgumentBytes = process.platform === "win32" ? 24_000 : 100_000;
...
async function writeAnimatedAvif(outputData, width, frameHeight, frameCount, outputPath, metadata) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dwif-avif-out-"));
  const frameStride = width * frameHeight * 4;
  const timing = getTimingModel(metadata, frameCount);
  let onFrame = null;
  ...
  try {
    ...
    for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
      ...
      for (let copyIndex = 0; copyIndex < repeatCount; copyIndex += 1) {
        const framePath = path.join(tempDir, `frame-${String(outputIndex + 1).padStart(6, "0")}.png`);
        await fs.writeFile(framePath, pngBuffer);
        outputIndex += 1;
      }
    ...
    await writeAnimatedAvifFromFrameFiles(path.join(tempDir, "frame-%06d.png"), outputPath, timing.fps);
  } finally {
    await fs.rm(tempDir, { recursive: true, force: true });
  }
}
...
async function writeAnimatedOutput(outputData, width, frameHeight, frameCount, outputPath, metadata) {
  ...
  if (extension === ".avif") {
    await writeAnimatedAvif(outputData, width, frameHeight, frameCount, outputPath, metadata);
    return;
  }

  throw new Error("Animated output currently supports only .webp, .avif, and .gif.");
}
...
export async function processImage({
  ...
  const avifProbe = isAvifLikeInput(inputPath) ? await probeAvifInput(inputPath) : null;
  const preparedInput = await prepareInputForProcessing(inputPath, avifProbe);

  try {
    if (preparedInput.kind === "frame-sequence") {
      return await processAnimatedFrameSequence({
        framePaths: preparedInput.framePaths,
        outputPath,
        manualTopStrip,
        manualRadius,
        onProgress,
        fps: preparedInput.fps
      });
    }

    const source = sharp(preparedInput.inputPath, SHARP_SOURCE_OPTIONS);
    ...
    if (frameCount > 1) {
      ...
      await writeStillOutput(preparedInput.inputPath, outputPath, metadata, topStrip, clampedRadius);
    }
